				...
				if ($OPTION['ArchiveStats'])
				{
					...
					while(($current_line = fscanf($this->handle, $this->archformat)) && !$timefound)
					{
						if ($current_line[0] == $real_time)
						{
							...
							$newline = $this->Create_Line($this->Format_Array(array($result_array[0], ($result_array[1] + $hits), ($result_array[2] + $visits)), $this->archdefault));
					...
					{
						fseek($this->handle, 0, SEEK_END);
						$newline = $this->Create_Line($this->Format_Array(array($time[$i], $hits, $visits), $this->archdefault));
						fwrite($this->handle, $newline);
					}
				}
			}
